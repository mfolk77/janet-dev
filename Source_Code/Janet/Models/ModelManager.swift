...
public class ModelManager: ObservableObject {
    ...
    /// Logger for the model manager
    private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "com.janet.ai", category: "ModelManager")
    
    /// The model instance
    private var model: (any JanetAIModel)?
    
    /// The model type
    private var modelType: JanetModelType = .ollama
    ...
    public func loadModel(type: JanetModelType = .ollama) async throws {
        ...
        // Create the model
        model = ModelFactory.createModel(type: type, modelPath: modelPath, tokenizerPath: tokenizerPath)
    ...
    ///   - tokenizerPath: The path to the tokenizer
    public func loadModelWithCustomPaths(type: JanetModelType = .ollama, modelPath: String, tokenizerPath: String) async throws {
    ...
    public func getModelType() -> JanetModelType {
        return modelType
    }
    ...
    /// Get a model by type
    /// - Parameter type: The type of model to get
    /// - Returns: The model instance, or nil if not found or not loaded
    public func getModel(type: JanetModelType) -> (any JanetAIModel)? {
        if type == modelType && isLoaded {
            return model
        }
        return nil
    }
    ...
    /// - Returns: Array of all loaded model types
    public func getLoadedModels() -> [JanetModelType] {
        if isLoaded {
            return [modelType]
        }
        return []
