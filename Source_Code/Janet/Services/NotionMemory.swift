...
class NotionMemory: ObservableObject {
    ...
    static let shared = NotionMemory()
    ...
    init() {
        ...
        $isEnabled
            .sink { newValue in
                ...
                if newValue && !(self.apiKey.isEmpty) && !(self.databaseId.isEmpty) {
                    Task {
                        do {
        ...
        $apiKey
            .sink { newValue in
                ...
                keychainManager.saveOrUpdateString(newValue, forKey: "notion_api_key")
            }
            .store(in: &cancellables)
        ...
        $databaseId
            .sink { newValue in
                // Save to keychain instead of UserDefaults
                keychainManager.saveOrUpdateString(newValue, forKey: "notion_database_id")
            }
            .store(in: &cancellables)
    ...
    /// Find relevant Notion items for a given query
    /// - Parameters:
    ///   - query: The search query
    ///   - limit: Maximum number of items to return
    /// - Returns: Array of relevant NotionItems
    func findRelevantNotionItems(query: String, limit: Int = 5) -> [NotionItem] {
        guard isEnabled, !notionItems.isEmpty else {
            return []
        }
        ...
        // Score each item based on matches in title and content
        let scoredItems = notionItems.map { item -> (item: NotionItem, score: Double) in
            var score = 0.0
            ...
            // Check content
            for word in queryWords {
                if item.content.lowercased().contains(word) {
                    score += 1.0
                }
            }
